...
#             # 그래프 출력
#             fig, ax = plt.subplots(figsize=(12, 4))
#             ax.plot(np.arange(len(processed_values)), processed_values)
#             ax.set_xlabel("Time")
#             ax.set_ylabel("RMS Value")
#             ax.set_title(f"RMS Data - {time.strftime('%Y-%m-%d %H:%M', current_time)}")
#             st.pyplot(fig)
...
while current_time <= end_time:
    ...
    if data:
        ...
        if processing_technique == "RMS":
            ...
            axes[0].clear()
            axes[0].plot(np.arange(len(processed_values)), processed_values)
            axes[0].set_xlabel("Time")
            axes[0].set_ylabel("Value")
            axes[0].set_title(f"Sensor 1 - {processing_technique} Data")
            ...
            for i, column in enumerate(processed_df.columns):
                axes[i].clear()
                axes[i].plot(np.arange(len(processed_values)), processed_df[column])
                axes[i].set_xlabel("Time")
                axes[i].set_ylabel("Value")
                axes[i].set_title(f"Sensor {i + 1} - {processing_technique} Data")
